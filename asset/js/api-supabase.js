...
async function loadUserAvatars() {
    const { data: rows, error } = await supabase.from('user_avatars').select('email, avatar_url, personal_column_slot');
    if (error) {
        console.error("Caricamento avatar/slot colonne personali fallito:", error);
        showToast("⚠️ Impossibile leggere i profili: colonne personali in sola lettura");
        return;
    }
    if (rows) {
        userAvatarsByEmail = {};
        personalColumnOwnerBySlot = {};
        rows.forEach(r => {
            userAvatarsByEmail[r.email] = r.avatar_url;
            if (r.personal_column_slot) personalColumnOwnerBySlot[r.personal_column_slot] = r.email;
        });
    }
}
...
async function loadPersonalColumnsSettings() {
    ...
    if (error) {
        console.error("Caricamento impostazioni colonne personali fallito:", error);
        showToast("⚠️ Titoli e legende delle colonne personali non caricati");
        return;
    }
    if (rows) {
...
async function loadProductsFromSupabase() {
    ...
    if (error) {
        console.error("Caricamento prodotti fallito:", error);
        showToast("⚠️ Impossibile caricare i prodotti: " + error.message);
        return;
    }
    if (productsData) {
...
async function loadTicketsFromSupabase() {
    ...
    if (error) {
        console.error("Caricamento ticket fallito:", error);
        showToast("⚠️ Impossibile caricare i ticket: " + error.message);
        return;
    }
    if (ticketsData) {
        localTickets = {};
        ...
        ticketsData.forEach(t => {
            const mapped = {
                ...
                productKeyStorica: t.product_key
            };
            if (t.product_id) {
                const chiave = `id:${t.product_id}`;
                if (!localTickets[chiave]) localTickets[chiave] = [];
                localTickets[chiave].push(mapped);
            } else if (t.product_key) {
                orphanTickets.push(mapped);
            } else {
...
async function updateProductInSupabase(rowObj, options = {}) {
    ...
    if (rowId) {
        const { data: updated, error } = await supabase.from('products').update({
            ...
            data: extraData,
            updated_at: new Date()
        }).eq('id', rowId).select('id, updated_at');
        if (error) throw error;
        if (updated && updated.length > 0) {
            rowObj.updatedAt = updated[0].updated_at;
            return;
        }

        if (!options.recreateIfMissing) {
            const missing = new Error("la riga non esiste più sul server (forse eliminata da un altro amministratore)");
            missing.code = 'ROW_MISSING';
            throw missing;
        }
    }
    ...
    const { data: newRow, error } = await supabase.from('products').insert([{
        ...
        in_catalogo: inCat,
        business_unit: currentDashboard,
        last_reviewed_at: rowObj.lastReviewedAt || new Date(),
        data: extraData
    }]).select().single();
    if (error) throw error;
    if (newRow) {
        ...
        rowObj.updatedAt = newRow.updated_at;
    }
}
