...
function forgetRowInHistory(row) {
    const key = historyRowKey(row);
    const purge = stack => stack.forEach((snapshot, i) => {
        stack[i] = snapshot.filter(r => historyRowKey(r) !== key);
    });
    purge(historyStack);
    purge(redoStack);
}
...
async function applyHistorySnapshot(targetData) {
    ...
    for (const [key, newRow] of afterMap) {
        ...
        if (!oldRow) {
            try { await updateProductInSupabase(newRow, { recreateIfMissing: true }); }
