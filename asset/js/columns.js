...
export function derivatiDiRiga(riga, ctx) {
    ...
    (sezioni['sec_3'] ? sezioni['sec_3'].childCols : []).forEach(c => {
        if (c === 'QR CODE ITA') { if (!web.isNo && !web.hasIt) docsIncompleto = true; }
        else if (c === 'QR CODE ENG') { if (!web.isNo && !web.hasEn) docsIncompleto = true; }
        else if (g('isSiNoColumn')(c)) { if (!g('rispostaSiNoCompleta')(riga[c])) docsIncompleto = true; }
    ...
    const visualIncompleto = (sezioni['sec_4'] ? sezioni['sec_4'].childCols : []).some(c => {
        ...
        if (g('isSiNoColumn')(c)) return !g('rispostaSiNoCompleta')(v);
        return v === '';
    });
...
const linkOppureNo = (icona, titolo) => (valore, riga, ctx, derivati) => {
    const testo = (valore || '').toString().trim();
    if (testo.toLowerCase() === 'no') return NIENTE;
    return cellaLinkOAvviso(testo, icona, titolo ? titolo(derivati) : null);
};
...
export const COLONNE = [
    ...
    { key: 'Datasheet Doc [ENG]', classes: ['col-btn-cell'], actions: azioniDi('Datasheet Doc [ENG]'), render: linkOppureNo('📝' + bandiera('ENG')) },
    { key: 'PDF Datasheet [ENG]', classes: ['col-btn-cell'], actions: azioniDi('PDF Datasheet [ENG]'), render: linkOppureNo('📑' + bandiera('ENG')) },
    ...
    { key: 'PDF Datasheet [ITA]', classes: ['col-btn-cell'], actions: azioniDi('PDF Datasheet [ITA]'), render: linkConIcona('📑' + bandiera('ITA')) },
    {
        ...
        render: (valore, riga, ctx, d) => qrCode(d.urlIt, d, 'QR CODE ITA', ctx.indiceDi(riga))
    },
    {
        key: 'QR CODE ENG', classes: ['col-narrow'], actions: NESSUNA_AZIONE,
        header: () => `<span class="th-label">QR CODE ENG ${bandiera('ENG')}</span>`,
        render: (valore, riga, ctx, d) => qrCode(d.urlEn, d, 'QR CODE ENG', ctx.indiceDi(riga))
    },
    { key: 'CE', label: 'CE & RoHS', classes: ['col-btn-cell'], actions: azioniDi('CE'), render: linkOppureNo('🇪🇺', d => 'CE_' + d.versione) },
    ...
    { key: 'Render Ambientali', classes: ['col-btn-cell'], actions: azioniDi('Render Ambientali'), render: linkOppureNo('🎨') },
...
function paginaWeb(valore, derivati, haLink, lingua) {
    if (derivati.isNo) return NIENTE;
...
function qrCode(indirizzo, derivati, colonna, indice) {
    if (derivati.isNo) return NIENTE;
