...
function editCell(btn, rowIndex, colName) {
    ...
    } else if (isSiNoColumn(colName)) {
        contentDiv.innerHTML = buildSiNoEditor(rowIndex, colName, currentValue);
        setTimeout(() => contentDiv.querySelector('select').focus(), 50);
    } else {
...
function buildSiNoEditor(rowIndex, colName, currentValue) {
    ...
    const etichettaNo = ORIGINALE_COLUMNS.includes(colName) ? 'No, non disponibile' : "No, non c'è";
    const opzione = (valore, etichetta) => `<option value="${valore}" ${scelta === valore ? 'selected' : ''}>${etichetta}</option>`;
    return `<div class="si-no-edit">
        <select class="standard-select" onchange="chooseSiNo(this, ${rowIndex}, '${colName}')">
            <option value="">-- Seleziona --</option>
            ${opzione('Sì', 'Sì, ho il link')}${opzione('No', etichettaNo)}
        </select>
        <input type="text" class="edit-input" placeholder="Incolla qui il link" value="${escapeHtml(link)}"
            onblur="saveSiNoLink(this, ${rowIndex}, '${colName}')"
            onkeypress="if(event.key === 'Enter') this.blur()" ${scelta === 'Sì' ? '' : 'hidden'}>
    </div>`;
}

async function chooseSiNo(select, rowIndex, colName) {
...
async function saveSiNoLink(input, rowIndex, colName) {
