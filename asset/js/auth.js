...
function updateUserAvatar(email, avatarUrl = null) {
    ...
    const CSS_URL_ESCAPES = { "'": '%27', '"': '%22', '(': '%28', ')': '%29' };
    const safeAvatarUrl = avatarUrl && /^https:\/\//i.test(avatarUrl)
        ? encodeURI(avatarUrl).replace(/['"()]/g, c => CSS_URL_ESCAPES[c])
        : null;

    if (safeAvatarUrl) {
        avatarBtn.style.backgroundImage = `url('${safeAvatarUrl}')`;
        dropdownAvatar.style.backgroundImage = `url('${safeAvatarUrl}')`;
