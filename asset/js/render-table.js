...
function isSiNoColumn(col) {
    return SI_NO_COLUMNS.includes(col);
}

function rispostaSiNoCompleta(valore) {
    ...
    data.forEach((row, idx) => {
        ...
        if (isCatBool) {
            ...
            sectionsConfig["sec_3"].childCols.forEach(c => {
                if (isNo && (c === "QR CODE ITA" || c === "QR CODE ENG")) return;
                totalRequiredFields++;
                if (c === "QR CODE ITA") { if (hasIt) totalCompletedFields++; }
                else if (c === "QR CODE ENG") { if (hasEn) totalCompletedFields++; }
                else if (isSiNoColumn(c)) { if (rispostaSiNoCompleta(row[c])) totalCompletedFields++; }
            ...
            sectionsConfig["sec_4"].childCols.forEach(c => {
                ...
                if (isSiNoColumn(c)) { if (rispostaSiNoCompleta(v)) totalCompletedFields++; }
...
function renderTable() {
    ...
    data.forEach((row, originalIndex) => {
        ...
        let hasMissingData = columns.some(col => {
            ...
            if (isSiNoColumn(col)) return !rispostaSiNoCompleta(v);
            return v === '';
        });
