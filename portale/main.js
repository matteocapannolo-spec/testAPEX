...
function disegnaCard(m) {
    ...
    const inBeta = m.beta === true;

    let marchio = '';
    if (inSviluppo) {
        marchio = '<span class="modulo-marchio">Coming soon</span>';
    } else if (inBeta) {
        marchio = '<span class="modulo-marchio beta">Beta</span>';
    }
    ...
    return `<a class="modulo-card" href="${escapeHtml(m.url)}">${marchio}${icona}${piede}</a>`;
}
